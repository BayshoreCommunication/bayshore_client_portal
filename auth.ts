...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  cookies: {
    sessionToken: {
      name: sessionCookieName(process.env.NODE_ENV === "production"),
      options: {
        httpOnly: true,
        sameSite: "lax",
        path: "/",
  ...
  providers: [
    Credentials({
      ...
      async authorize(credentials) {
        ...
        let res: Response;
        try {
          res = await fetch(`${BACKEND_API_URL}/auth/client/signin`, {
            method: "POST",
        ...
        // /auth/client/signin already refuses every other role. Check again here
        // so this portal stays client-only even if that route ever changes, and
        // revoke the refresh token the backend just issued.
        if (user.role !== "client") {
