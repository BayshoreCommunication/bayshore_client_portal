...
export default async function proxy(req: NextRequest) {
  ...
  const cookieName = sessionCookieName(req.nextUrl.protocol === "https:");
  ...
  if (
    ...
    if (outcome.status === "ok" && outcome.tokens.profile?.role && outcome.tokens.profile.role !== CLIENT_ROLE) {
      // The account is no longer a client — end the session here.
      isLoggedIn = false;
